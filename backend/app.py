...
# Database - Use the instance folder database that has training services
basedir = os.path.abspath(os.path.dirname(__file__))
project_root = os.path.dirname(basedir)  # Go up one level from backend/ to project root
database_path = os.path.join(project_root, 'instance', 'simple_solimicrolink.db')

# Use DATABASE_URL from environment if set (for production), otherwise use the instance database
...
if os.path.exists(database_path):
    print(f"📁 Database file size: {os.path.getsize(database_path)} bytes")
...
if IS_RENDER:
    ...
    UPLOAD_FOLDER = '/opt/render/project/src/frontend/static/uploads'
else:
    # Local development - use local folder
    basedir = os.path.abspath(os.path.dirname(__file__))
...
def admin_upload_owner_photo():
    """Upload owner photo for About Us page"""
    try:
...
def admin_upload_teacher_photo():
    ...
    try:
        ...
        filename = secure_filename(f"teacher_{teacher_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}")
        filepath = os.path.join(UPLOAD_FOLDER, filename)
        file.save(filepath)
...
def admin_update_settings():
    ...
    try:
        ...
        footer_settings = {
            ...
            'footer_email': request.form.get('footer_email', 'seluweldu96@gmail.com'),
